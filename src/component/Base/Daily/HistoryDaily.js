...
export default function HistoryDaily({open, close}) {
    ...
    return (
        <Modal open={open} close={close}>
            <div className="p-2 flex flex-col w-full">
                {task? <DetailTask task={task} resetTask={() => setTask(null)}/> : <ListTask history={source?.history} cb={(task) => setTask(task)} />}
            </div>
        </Modal>
    )
}
...
function BoxList({item}) {
    ...
    return (
        <div className='bg-zinc-800 rounded p-2 flex items-center px-4 flex-col pointer last:mb-2' onClick={() => setDetail(prev => !prev)}>
